...
const FeedbackPage = ({ route }) => {
  ...
  const handleFeedbackSubmit = async () => {
    if (!sessionToken) {
      setMessage('Session token is missing. Please log in again.');
      return;
    }
    const response = await submitFeedback(sessionToken, feedback, photo);
    if (response.success) {
      setMessage('Feedback submitted successfully');
      setFeedback('');
      setPhoto(null);
    } else {
      setMessage('Failed to submit feedback: ' + response.message);
    }
  };
  return (
